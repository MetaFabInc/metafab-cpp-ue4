...
bool OpenAPIPlayersApi::IsValid() const
{
	if (Url.IsEmpty())
	{
		UE_LOG(LogOpenAPI, Error, TEXT("OpenAPIPlayersApi: Endpoint Url is not set, request cannot be performed"));
		return false;
	}
...
FHttpRequestPtr OpenAPIPlayersApi::RemovePlayerConnectedWallet(const RemovePlayerConnectedWalletRequest& Request, const FRemovePlayerConnectedWalletDelegate& Delegate /*= FRemovePlayerConnectedWalletDelegate()*/) const
{
	if (!IsValid())
		return nullptr;
	...
	HttpRequest->OnProcessRequestComplete().BindRaw(this, &OpenAPIPlayersApi::OnRemovePlayerConnectedWalletResponse, Delegate);
	HttpRequest->ProcessRequest();
	return HttpRequest;
}

void OpenAPIPlayersApi::OnRemovePlayerConnectedWalletResponse(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, FRemovePlayerConnectedWalletDelegate Delegate) const
{
	RemovePlayerConnectedWalletResponse Response;
	HandleResponse(HttpResponse, bSucceeded, Response);
	Delegate.ExecuteIfBound(Response);
}

FHttpRequestPtr OpenAPIPlayersApi::SetPlayerConnectedWallet(const SetPlayerConnectedWalletRequest& Request, const FSetPlayerConnectedWalletDelegate& Delegate /*= FSetPlayerConnectedWalletDelegate()*/) const
{
	if (!IsValid())
		return nullptr;
	...
	HttpRequest->OnProcessRequestComplete().BindRaw(this, &OpenAPIPlayersApi::OnSetPlayerConnectedWalletResponse, Delegate);
	HttpRequest->ProcessRequest();
	return HttpRequest;
}

void OpenAPIPlayersApi::OnSetPlayerConnectedWalletResponse(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, FSetPlayerConnectedWalletDelegate Delegate) const
{
	SetPlayerConnectedWalletResponse Response;
	HandleResponse(HttpResponse, bSucceeded, Response);
	Delegate.ExecuteIfBound(Response);
}
